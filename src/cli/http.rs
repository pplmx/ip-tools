    ...
    run_probe_flow(
        ...
        move |host, dest, timeout| {
            ...
            async move {
                ...
                if insecure {
                    ip_http::probe_insecure(dest, &host, &method, &path, &header_refs, body.as_deref(), timeout).await
                } else {
                    ip_http::probe(dest, &host, &method, &path, &header_refs, body.as_deref(), timeout).await
                }
            }
        },
    )
